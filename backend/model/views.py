...
END_DATE_M = datetime.strptime('2022-11-20', '%Y-%m-%d') # 종료 시점
...
feature_path = os.getcwd() + "/routes/market/data/" + fg.range + "_feature.csv"
label_path = os.getcwd() + "/routes/market/data/" + fg.range + "_label.csv"
result_path = os.getcwd() + "/routes/market/result/" + fg.range + "_"
...
point_path = os.getcwd() + "/market/result/" + fg.range + "_100.json"
...
def market_model(request, point_num):
    ...
    feature = pd.read_csv("./market/data/" + fg.range + "_feature.csv", index_col = 0)
    label = pd.read_csv("./market/data/" + fg.range + "_label.csv", index_col = 0)
    ...
    return JsonResponse({"result" : "Market model finished."}, safe=False)
...
def sector_model_all(request):
    ...
    if os.path.isfile(os.getcwd() + "/sector/result/" + "all" + "_result.csv"):
        csv_data = pd.read_csv(os.getcwd() + "/sector/result/" + "all" + "_result.csv", sep = ",")
        json_data = csv_data.to_json(orient = "records")
        return json_data
    ...
    df.to_csv(os.getcwd() + "/sector/data/" + "all" + "_data.csv")
    return JsonResponse({"result" : "Sector model(sector_code) finished."}, safe=False)
